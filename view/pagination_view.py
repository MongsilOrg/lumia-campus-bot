...
def basic_build_table(result, page, page_size=20):
...
def product_build_table(result, page, page_size=20):
    table = f"{'번호':<5} | {'구매자':<20}\n"
    table += "-" * 30 + "\n"
    for i, item in enumerate(result, start=(page - 1) * page_size + 1):
        table += f"{i:<6} | {item[0]:<20}\n"
    return table
...
class PaginationView(discord.ui.LayoutView):
    ...
    async def _update(self, interaction: discord.Interaction):
        ...
        table_text = basic_build_table(result, self.page)
        new_view = PaginationView(self.user_id, self.page, self.total, table_text)
        await interaction.response.edit_message(view=new_view)


class ProductPaginationView(discord.ui.LayoutView):
    def __init__(self, product_name, current_page, total_pages, table_text=""):
        super().__init__(timeout=60)
        ...
        container = discord.ui.Container(accent_colour=discord.Colour.blurple())
        container.add_item(discord.ui.TextDisplay(
            f"## 📋 {product_name} 구매자 목록\n"
            f"```\n{table_text}```\n"
            f"-# 페이지 {current_page} / {total_pages}"
        ))
        row = discord.ui.ActionRow()
        prev_btn = discord.ui.Button(label="이전", style=discord.ButtonStyle.primary, disabled=(current_page <= 1))
        next_btn = discord.ui.Button(label="다음", style=discord.ButtonStyle.primary, disabled=(current_page >= total_pages))
        async def on_prev(interaction: discord.Interaction):
            if self.page > 1:
                self.page -= 1
                await self._update(interaction)
    ...
    async def _update(self, interaction: discord.Interaction):
        ...
        table_text = product_build_table(result, self.page)
        new_view = ProductPaginationView(self.product_name, self.page, total_pages, table_text)
        await interaction.response.edit_message(view=new_view)