...
class PointSystem(commands.Cog):
    ...
    async def remove_point_cmd(self, interaction: discord.Interaction, member : discord.Member , points: int):
        ...
        if result is None:
            await interaction.response.send_message("사용자가 존재하지 않습니다.", ephemeral=True)
            return
        await interaction.response.send_message(f"사용자 {member.nick}에게 {points}점이 차감되었습니다.", ephemeral=True)
    ...
    async def update_point_cmd(self, interaction: discord.Interaction, member : discord.Member , points: int):
        ...
        if result is None:
            await interaction.response.send_message("사용자가 존재하지 않습니다.", ephemeral=True)
            return
        await interaction.response.send_message(f"사용자 {member.nick}의 포인트가 {points}점으로 업데이트되었습니다.", ephemeral=True)
